...
```{r}
data %>%
  gg_tsdisplay(Abflussmenge, plot_type='partial') #muss das nicht hinter fill_gaps geschoben werden? ansonsten entsteht ein error?
```
Notes:
...
```{r}
autoplot(decomposition_stl) # decomposition wurde in decompostion_stl umbenannt
```
...
```{r}
#is heteroscedastic
remainder <- decomposition_stl$remainder #in _stl umbennant
acf(remainder)
```
