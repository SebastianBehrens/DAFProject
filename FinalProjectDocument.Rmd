...
fits %>% 
  select(stepwise) %>%
  gg_tsresiduals()
#brauchen das für den unteren blind fit
...
Forecast_arima_blindFit  %>% report()
Forecast_arima_blindFit %>% forecast(h=8800) %>% autoplot(data)


#generate forecast for next 8800*5min = 44'000min = 30.5 days
b <- Forecast_arima_blindFit %>% generate(h=8800)
